...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id) {
        ...
        $sql = "UPDATE steps1 SET title = :title, description = :description, option_yes_id = :option_yes_id, option_no_id = :option_no_id WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $id);
    } else {
        // Crear nuevo paso
        $sql = "INSERT INTO steps1 (title, description, option_yes_id, option_no_id) VALUES (:title, :description, :option_yes_id, :option_no_id)";
        $stmt = $pdo->prepare($sql);
    }
