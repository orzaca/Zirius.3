...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_flow'])) {
    ...
    if (empty($name)) {
        ...
        if ($stmt->execute([':name' => $name, ':description' => $description])) {
            header('Location: create_gpon.php'); // Redirigir a la misma página después de guardar
            exit;
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_flow'])) {
    ...
    if (empty($id) || empty($name)) {
        ...
        $sql = "UPDATE flows SET name = :name WHERE id = :id";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':name' => $name, ':id' => $id])) {
            header('Location: create_gpon.php'); // Redirigir después de actualizar
            exit;
        } else {
...
$sql = "SELECT id, name, description FROM flows ORDER BY id ASC";
$stmt = $pdo->query($sql);
...
<body>
    <div class="container">
        ...
        <form method="POST" action="create_gpon.php">
