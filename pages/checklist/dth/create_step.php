...
$sql_steps = "SELECT * FROM steps2";  // **Cambio aquí: "steps" a "steps1"**
$stmt_steps = $pdo->prepare($sql_steps);
$stmt_steps->execute();
$steps = $stmt_steps->fetchAll(PDO::FETCH_ASSOC);

// Obtener los flujos existentes
$sql_flows = "SELECT * FROM flows2";  // **Cambio aquí: "flows" a "flows1"**
...
if (isset($_GET['id'])) {
    ...
    $id = $_GET['id'];
    $sql = "SELECT * FROM steps2 WHERE id = :id";  // **Cambio aquí: "steps" a "steps1"**
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id) {
        ...
        $sql = "UPDATE steps2 SET title = :title, description = :description, option_yes_id = :option_yes_id, option_no_id = :option_no_id, flow_id = :flow_id WHERE id = :id";  // **Cambio aquí: "steps" a "steps1"**
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $id);
    } else {
        // Crear nuevo paso
        $sql = "INSERT INTO steps2 (title, description, option_yes_id, option_no_id, flow_id) VALUES (:title, :description, :option_yes_id, :option_no_id, :flow_id)";  // **Cambio aquí: "steps" a "steps1"**
        $stmt = $pdo->prepare($sql);
    }
