...
$sql_flows = "SELECT id, name FROM flows2";  // **Cambio: 'flows' a 'flows1'**
$stmt_flows = $pdo->prepare($sql_flows);
$stmt_flows->execute();
$flows = $stmt_flows->fetchAll(PDO::FETCH_ASSOC);

// Consultar los pasos para el flujo seleccionado
$sql_steps = "SELECT * FROM steps2 WHERE flow_id = :flow_id ORDER BY id";  // **Cambio: 'steps' a 'steps1'**
...
if ($current_step_id === null) {
    $current_step = $steps[0]; // Primer paso del flujo
} else {
...
function get_step_by_id($pdo, $step_id, $flow_id) {
    $sql = "SELECT * FROM steps2 WHERE id = :id AND flow_id = :flow_id";  // **Cambio: 'steps' a 'steps1'**
