...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_flow'])) {
    ...
    if (empty($name)) {
        ...
        $sql = "INSERT INTO flows2 (name, description) VALUES (:name, :description)";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':name' => $name, ':description' => $description])) {
            header('Location: create_dth.php'); // Redirigir a la misma página después de guardar
            exit;
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_flow'])) {
    ...
    if (empty($id) || empty($name)) {
        ...
        $sql = "UPDATE flows2 SET name = :name WHERE id = :id";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':name' => $name, ':id' => $id])) {
            header('Location: create_dth.php'); // Redirigir después de actualizar
            exit;
        } else {
...
$sql = "SELECT id, name, description FROM flows2 ORDER BY id ASC";
$stmt = $pdo->query($sql);
...
<body>
    <div class="container">
        ...
        <form method="POST" action="create_dth.php">
