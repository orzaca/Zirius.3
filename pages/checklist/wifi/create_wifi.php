...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_flow'])) {
    ...
    if (empty($name)) {
        ...
        $sql = "INSERT INTO flows6 (name, description) VALUES (:name, :description)";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':name' => $name, ':description' => $description])) {
            header('Location: create_wifi.php'); // Redirigir a la misma página después de guardar
            exit;
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_flow'])) {
    ...
    if (empty($id) || empty($name)) {
        ...
        $sql = "UPDATE flows6 SET name = :name WHERE id = :id";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([':name' => $name, ':id' => $id])) {
            header('Location: create_wifi.php'); // Redirigir después de actualizar
            exit;
        } else {
...
$sql = "SELECT id, name, description FROM flows6 ORDER BY id DESC";
$stmt = $pdo->query($sql);
...
<body>
    <div class="container">
        ...
        <form method="POST" action="create_wifi.php">
