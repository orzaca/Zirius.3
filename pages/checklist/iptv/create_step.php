...
if (isset($_GET['id'])) {
    ...
    $id = $_GET['id'];
    $sql = "SELECT * FROM steps4 WHERE id = :id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id) {
        ...
        $sql = "UPDATE steps4 SET title = :title, description = :description, option_yes_id = :option_yes_id, option_no_id = :option_no_id, flow_id = :flow_id WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        $stmt->bindParam(':id', $id);
    } else {
        // Crear nuevo paso
        $sql = "INSERT INTO steps4 (title, description, option_yes_id, option_no_id, flow_id) VALUES (:title, :description, :option_yes_id, :option_no_id, :flow_id)";
        $stmt = $pdo->prepare($sql);
    }
