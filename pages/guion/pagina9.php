...
<body>
    ...
    <div class="content-wrapper">
        ...
        foreach ($paragraphs as $para) {
            echo '<div class="paragraph-box" id="para9' . $para['id'] . '">'; // Cambiado a para2
            echo '<p>' . htmlspecialchars($para['content']) . '</p>';
            echo '<div class="button-group">'; // Nuevo contenedor para los botones
            echo '<button class="copy-btn"  onclick="copyText(\'para9' . $para['id'] . '\')">Copiar</button>';// Cambiado a para2
            echo '<button  class="edit-btn" onclick="openEditModal9(' . $para['id'] . ')">Modificar</button>';
            echo '</div>';
            echo '</div>';
        }
        ?>
    ...
    <div id="editModal9"> <!-- Cambiado a editModal3 -->
        <div class="modal-content">
            <span class="close-btn" onclick="closeEditModal9()">&times;</span>
            <h2>Editar Párrafo</h2>
            <textarea id="editText9" rows="13" style="width: 100%;"></textarea> <!-- Cambiado a editText3 -->
            <button id="saveChanges9" onclick="saveParagraph9()">Guardar</button> <!-- Cambiado a saveChanges3  -->
        </div>
    </div>
