...
<body>
    ...
    <div class="content-wrapper">
        ...
        foreach ($paragraphs as $para) {
            echo '<div class="paragraph-box" id="para8' . $para['id'] . '">'; // Cambiado a para2
            echo '<p>' . htmlspecialchars($para['content']) . '</p>';
            echo '<div class="button-group">'; // Nuevo contenedor para los botones
            echo '<button class="copy-btn" onclick="copyText(\'para8' . $para['id'] . '\')">Copiar</button>';// Cambiado a para2
            echo '<button class="edit-btn"  onclick="openEditModal8(' . $para['id'] . ')">Modificar</button>';
            echo '</div>';
            echo '</div>';
        }
        ?>
    ...
    <div id="editModal8"> <!-- Cambiado a editModal3 -->
        <div class="modal-content">
            <span class="close-btn" onclick="closeEditModal8()">&times;</span>
            <h2>Editar Guion</h2>
            <textarea id="editText8" rows="13" maxlength="500"  style="width: 100%;"></textarea>
	         <p id="charCount">Solo puedes escribir hasta 500 caracteres</p>
            <button id="saveChanges8" onclick="saveParagraph8()">Guardar</button> <!-- Cambiado a saveChanges3  -->
        </div>
    </div>
