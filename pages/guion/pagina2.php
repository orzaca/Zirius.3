...
<body>
    ...
    <div class="content-wrapper pagina2">
    <?php
    // Mostrar cada párrafo en la interfaz
    foreach ($paragraphs as $para) {
        echo '<div class="paragraph-box" id="para2' . $para['id'] . '">';
        echo '<p>' . htmlspecialchars($para['content']) . '</p>';
        echo '<div class="button-group">'; // Nuevo contenedor para los botones
        echo '<button class="copy-btn" onclick="copyText(\'para2' . $para['id'] . '\')">Copiar</button>';
        echo '<button class="edit-btn" onclick="openEditModal2(' . $para['id'] . ')">Modificar</button>';
...
<div id="editModal2"> <!-- Cambiado a editModal2 -->
    <div class="modal-content">
        <span class="close-btn" onclick="closeEditModal2()">&times;</span>
        <h2>Editar Guion</h2>
        <textarea id="editText2" rows="13" maxlength="500"  style="width: 100%;"></textarea>
	         <p id="charCount">Solo puedes escribir hasta 500 caracteres</p>
        <button id="saveChanges2" onclick="saveParagraph2()">Guardar</button>
    </div>
</div>
