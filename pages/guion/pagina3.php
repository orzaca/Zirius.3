...
$sql = "SELECT p.id, COALESCE(up.content, p.content) AS content FROM pagina3 p 
        LEFT JOIN user_pagina3 up ON p.id = up.pagina3_id AND up.user_id = :userId";
...
<body>
    ...
    <div class="content-wrapper">
        ...
        foreach ($paragraphs as $para) {
            echo '<div class="paragraph-box" id="para3' . $para['id'] . '">'; // Cambiado a para2
            echo '<p>' . htmlspecialchars($para['content']) . '</p>';
            echo '<div class="button-group">'; // Nuevo contenedor para los botones
            echo '<button class="copy-btn" onclick="copyText(\'para3' . $para['id'] . '\')">Copiar</button>';// Cambiado a para2
            echo '<button class="edit-btn" onclick="openEditModal3(' . $para['id'] . ')">Modificar</button>';
            echo '</div>';
            echo '</div>';
        }
        ?>
    ...
    <div id="editModal3"> <!-- Cambiado a editModal3 -->
        <div class="modal-content">
            <span class="close-btn" onclick="closeEditModal3()">&times;</span>
            <h2>Editar Párrafo</h2>
            <textarea id="editText3" rows="13" maxlength="500"  style="width: 100%;"></textarea>
	         <p id="charCount">Solo puedes escribir hasta 500 caracteres</p>
            <button id="saveChanges3" onclick="saveParagraph3()">Guardar</button> <!-- Cambiado a saveChanges3  -->
        </div>
    </div>
